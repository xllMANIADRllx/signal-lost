...
class BannerManager{
  constructor(scene){
    ...
    this._BW=W-this._LP-this._RP; // banner width = play area
    this._BX=this._LP;            // banner left edge
    this._PH=46;  // panel height
    this._GAP=3;  // gap between stacked banners
    this._MAX=4;
    this._startY=8; // top of first banner
  }

  show(text,color='#00cc66',duration=1800,sub=''){
    if(!this.scene||!this.scene.add)return;
    const col=parseInt(color.replace('#',''),16);
    const BX=this._BX, BW=this._BW, PH=this._PH, GAP=this._GAP;
    ...
    const timeT=this.scene.add.text(BX+BW-12,y+6,typeTag!=='SYS'?'':'',{fontFamily:"'Courier New',monospace",fontSize:'8px',color:'#556655'}).setOrigin(1,0).setDepth(102);
    const objs=[bg,tbar,bbar,tag,msg,timeT];if(subT)objs.push(subT);
